# AGENTS.md - AI Assistant Guide for RoundTimer

> **Last Updated**: 2026-03-10
> **Project**: RoundTimer - Kotlin Multiplatform Round Timer Application
> **Package**: `net.solvetheriddle.roundtimer`

This document serves as the primary technical guide and set of mandates for all AI assistants (Claude, Gemini, etc.) working on the RoundTimer project.
...
- Customizable audio cues (30+ WAV files, ~15MB total)
...
- 20+ overtime voice cues (Jonas scolding feature)

> **Design Decision: Resource Duplication**
> Sound files are duplicated in `androidMain/res/raw/` to support Android's native **`SoundPool`** API. While Compose Resources are used for iOS/JVM/Web, the Android implementation uses native resources to ensure the lowest possible latency for time-critical audio cues. Any new sound must be added to **both** locations and updated in the `Sound` and `AndroidSound` enums.
...
- Agent branches: `agent/guide-update-*`
...
For questions or updates to this document, modify `/AGENTS.md` directly.
